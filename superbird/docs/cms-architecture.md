...
1. **Collections model in-app** — `collections`/`fields`/`entries` as stores (seeded, pre-persistence); Pages\|Collections dropdown + item submenu; bind templates to collections; preview-entry selection; Option-A inline editing writing to entries. Retire the hardcoded configs.
2. **Persistence** — SQLite + API; stores become API-backed; media as files+paths.
