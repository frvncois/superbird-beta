...
- `src/stores/` — Domain Pinia stores (composition API `defineStore` with setup function): `canvas`, `globalStyles`, `media`, `siteSettings`, `locales`, `userComponents`, `collections`, `submissions`, `comments`, `snapshots`, `setup`, `auth`, `mcp`. All mutations go through actions (undo/redo depends on it).
