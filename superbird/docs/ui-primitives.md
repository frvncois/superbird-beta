...
| `SegmentedControlUi` | `string` | `options: {value,label?,title?,icon?,tooltip?}[]`, `size` `default`\|`xs`, `grow`=false, `tooltipPlacement`=`top` | `#option` slot `{option,active}` for custom content (e.g. icons); `grow` stretches segments; a per-option `tooltip` shows a `TooltipUi` on hover (and suppresses the native `title`) |
...
| `UnitInputUi` | `string` | `placeholder`, `units`=`[px,%,em,rem,vw,vh]`, `allowAuto`=false, `tokens?` | number + unit dropdown; arrow-key stepping (`lib/unitValue`). Typing a unit inline (e.g. `90em`) moves it to the dropdown and leaves just the number in the field. When size tokens are available (`tokens` prop or injected `GlobalTokensKey`) it also shows a token picker + badge and emits `var(--global-size-<name>)`; token UI is inert when no tokens exist, so it doubles as a plain unit input |
...
| `OverlayHostUi` | — (driven by `useOverlayStack`) | none | mounted **once** in `App.vue`. The single fixed overlay surface: two layers (`#overlay-modals` z-100, `#overlay-dialogs` z-105), one blur backdrop per layer, and the global Escape / backdrop-click dismissal (top of stack, respects `dismissible`). Never used directly |
| `ModalUi` | `open` (`v-model:open`) | `variant` `modal`\|`dialog`, `title`, `description`, `icon`, `danger`, `position` `center`\|`right`, `size` `sm`\|`md`\|`lg`\|`xl`, `closable`, `dismissible`, `panelClass`, `bodyClass` | teleports its panel into `OverlayHostUi` **only while open** (closed = nothing in the DOM; backdrop/Escape live in the host via `useOverlayStack`); `right`=full-height drawer. `dialog` variant = compact centered confirm/prompt (icon chip + `#actions` footer, red chip when `danger`); `modal` = titled panel with close (X). Slots: default (body), `#header`, `#header-action`, `#icon`, `#actions`. **This is the only modal/dialog primitive** — confirm/prompt/process flows are inline `<ModalUi variant="dialog">` driven by local `ref` state (there is no imperative dialog service) |
...
| `TooltipUi` | — | `content`, `placement` `top`(default)\|`bottom`\|`left`\|`right`, `delay`=300, `disabled`=false | wraps a trigger (default slot); teleports a fixed-positioned hover/focus tip to `<body>` (never clipped). Renders **no wrapper element** when `content` is empty/`disabled`, so it's zero-cost on triggers without a tip. Fallthrough `class` lands on the wrapper (e.g. `flex-1`) |
...
| `TabsUi` | `tabs: {key,label}[]`, v-model `string` | full-height column: pinned tab bar + flex-1 scrolling content pane w/ slide transition; one named slot per tab key. `inheritAttrs: false` — forwarded attrs (e.g. `data-sidebar-scroll`) land on the content scroller |
